...
# Take all lines and split into array
# take all integers from lines and insert into individual arrays within an array. (remember they are strings)
# conditional to  combine first and last...corner case, solo digit is itself twice.
# push these digits to array and sum. 

coord_integers = []

garbage.each do |trash|
  string_nums = []
  trash.split("").each do |char|
    if char.to_i > 0
      string_nums << char.to_i
    end
  end
  coord_integers << string_nums
end

sum_coords = 0
coord_integers.each do |array|
  sum_coords += ([array[0], array[-1]].join().to_i)
end
